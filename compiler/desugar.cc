...
void check_func_context_arg(type_system& ts, ast_node& func) {
    ...
    if (cond_callconv && !(func.desugar_flags & desugar_flag::has_context_arg)) {
        auto& args = func.func_args();
        auto arg_decl = make_var_decl_of_type(ts, token_type::let, CONTEXT_SYMBOL, ts.context_ptr_type);
        args.insert(args.begin(), std::move(arg_decl));
...
void check_call_context_arg(type_system& ts, ast_node& call) {
    ...
    if (cond_callconv && !(call.desugar_flags & desugar_flag::has_context_arg)) {
        ...
        args.insert(args.begin(), make_identifier_node(*ts.ast_arena, call.pos, { CONTEXT_SYMBOL }));
...
void check_var_decl_aggregate_call(type_system& ts, ast_node& node) {
    if (is_aggregate_type(node.tid) && node.var_value() && node.var_value()->type == ast_type::call_expr) {
        ...
        node.children[ast_node::child_var_decl_value] = make_undefined_expr_node(*ts.ast_arena, node.pos);
