...
# ir_deref L0; (push)
 ...
 cmp 8(%rax),%rbx
 jge .if2675$else
# ir_jmp_gte POP() [POP() . 1] .if2675$else;

.if2675$body:
# ir_make_label .if2675$body;
...
# ir_load L6 [L4 . 1];
 ...
 cmp $0,%r10d
 je .if2734$else
# ir_jmp_eq L6 0 .if2734$else;

.if2734$body:
# ir_make_label .if2734$body;
...
.w2851$cond:
# ir_make_label .w2851$cond;

 mov -8(%rbp),%r10
 cmp $0,%r10
 je .w2851$end
# ir_jmp_eq L0 0 .w2851$end;

.w2851$body:
# ir_make_label .w2851$body;
...
# ir_add L2 [POP() . 1]; (push)
 ...
 cmp %rax,%r10
 jge .if2838$else
# ir_jmp_gte A0 POP() .if2838$else;

.if2838$body:
# ir_make_label .if2838$body;

 jmp .w2851$end
# ir_jmp .w2851$end;

.if2838$else:
# ir_make_label .if2838$else;
...
.w2851$end:
# ir_make_label .w2851$end;

 mov -8(%rbp),%r10
 cmp $0,%r10
 jne .if2859$else
# ir_jmp_neq L0 0 .if2859$else;

.if2859$body:
# ir_make_label .if2859$body;

 jmp .cb__Nstd__Nmem__Nfree__Aptr__Topaque$end
# ir_return #0;

.if2859$else:
# ir_make_label .if2859$else;
...
# ir_deref L0; (push)
 ...
 cmp $0,%r10
 jne .if2902$else
# ir_jmp_neq [POP() . 2] 0 .if2902$else;

.if2902$body:
# ir_make_label .if2902$body;

 mov std__mem__last_block(%rip),%r10
 cmp -8(%rbp),%r10
 jne .if2891$else
# ir_jmp_neq  L0 .if2891$else;

.if2891$body:
# ir_make_label .if2891$body;
...
# ir_load L3 [L1 . 1];
 ...
 cmp $0,%r10d
 je .if4167$else
# ir_jmp_eq L3 0 .if4167$else;

.if4167$body:
# ir_make_label .if4167$body;
